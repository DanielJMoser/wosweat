...
// Element type for Cheerio
type CheerioElement = any; // Using any for Element as cheerio doesn't export it directly
...
function extractArtilleryEvents($: cheerio.CheerioAPI, url: string): EventData[] {
    ...
    // Find each product
    $('.product').each((index: number, element: CheerioElement) => {
        try {
...
function extractBaeckereiEvents($: cheerio.CheerioAPI, url: string): EventData[] {
    ...
    $(selectors.eventContainer).each((index: number, element: CheerioElement) => {
        try {
    ...
    $(selectors.recurringEventContainer).each((index: number, element: CheerioElement) => {
        try {
            const eventElement = $(element);
